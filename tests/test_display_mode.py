...
def test_display_binding_does_not_replace_gui_or_hivemind():
    from tui.app import VoxTerm

    bindings = {binding.action: binding for binding in VoxTerm.BINDINGS}

    assert bindings["open_display"].key == "shift+g"
    assert bindings["launch_gui"].key == "g"
    assert bindings["show_hivemind"].key == "h"
